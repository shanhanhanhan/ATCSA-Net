...
Cascade topology is the Phase-II refined operational hierarchy
(Algorithm 1 initializes {0,2,4}/{1,3} from validation C; refinement
freezes the binary experts below before test evaluation):
  coarse groups: {0,4} | {2,3} | {1}
  fine S2: class 0 vs 4 (Manual Digging vs Directional Drill)
  fine S1: class 2 vs 3 (Harvester vs Rotary Tiller)

Every node uses the same ATCSA backbone (CNN + CCSTA from arXiv:2409.19764).
"""
...
def save_cascade(path, model, extra=None):
    payload = {'state_dict': model.state_dict(), 'use_ccsta': model.use_ccsta}
    if extra:
        payload.update(extra)
    torch.save(payload, path)
...
def train_cascade_joint(model, train_loader, val_loader, num_epochs, save_path):
    """Path-wise hierarchical loss (paper Eq. 13) with teacher forcing."""
    ...
    print('\nTraining joint ACGC cascade (shared trunk + 3 heads) ...')
    print('=' * 80)
    for epoch in range(num_epochs):
        model.train()
        running_loss = 0.0
        n_seen = 0
        correct = 0
        pbar = tqdm(train_loader, desc=f'Cascade {epoch + 1}/{num_epochs}', leave=False)
        for images, orig in pbar:
            images = images.to(DEVICE)
            orig = orig.to(DEVICE)
            optimizer.zero_grad()
            out = model.forward_heads(images)
            ...
            coarse_y = torch.tensor(
                [original_to_coarse(int(y)) for y in orig.cpu().tolist()],
                device=DEVICE, dtype=torch.long,
            )
            loss = LAMBDA_COARSE * ce(out['coarse'], coarse_y)

            s1_mask = (orig == FINE_S1_CLASSES[0]) | (orig == FINE_S1_CLASSES[1])
            if s1_mask.any():
                s1_y = torch.where(orig[s1_mask] == FINE_S1_CLASSES[1], 1.0, 0.0)
                loss = loss + LAMBDA_S1 * bce(out['s1'][s1_mask], s1_y)

            s2_mask = (orig == FINE_S2_CLASSES[0]) | (orig == FINE_S2_CLASSES[1])
            if s2_mask.any():
                s2_y = torch.where(orig[s2_mask] == FINE_S2_CLASSES[1], 1.0, 0.0)
                loss = loss + LAMBDA_S2 * bce(out['s2'][s2_mask], s2_y)
                loss = loss + TRIPLET_WEIGHT * batch_hard_triplet_loss(
                    out['metric'][s2_mask], orig[s2_mask], margin=TRIPLET_MARGIN
                )
            ...
            groups = out['coarse'].argmax(dim=1)
            s1_bit = (out['s1'] > 0).long()
            s2_bit = (out['s2'] > 0).long()
            pred = torch.full_like(orig, LEAF_CLASS)
            is_g0 = groups == 0
            is_g1 = groups == 1
            pred[is_g0] = torch.where(
                s2_bit[is_g0] == 1,
                torch.full_like(pred[is_g0], FINE_S2_CLASSES[1]),
                torch.full_like(pred[is_g0], FINE_S2_CLASSES[0]),
            )
            pred[is_g1] = torch.where(
                s1_bit[is_g1] == 1,
                torch.full_like(pred[is_g1], FINE_S1_CLASSES[1]),
                torch.full_like(pred[is_g1], FINE_S1_CLASSES[0]),
            )

            bs = images.size(0)
            running_loss += loss.item() * bs
            n_seen += bs
            correct += (pred == orig).sum().item()
            pbar.set_postfix(loss=f'{loss.item():.4f}',
                             acc=f'{100.0 * correct / n_seen:.2f}%')
        ...
        train_loss = running_loss / max(n_seen, 1)
        train_acc = 100.0 * correct / max(n_seen, 1)
        val_coarse, val_acc, _ = eval_cascade_split(model, val_loader)
        history['train_loss'].append(train_loss)
        ...
        print(f'Epoch {epoch + 1}/{num_epochs}: '
              f'train {train_loss:.4f}/{train_acc:.2f}%  '
              f'val 5-class {val_acc:.2f}%  coarse {val_coarse:.2f}%')
        if val_acc >= best_val_acc:
            best_val_acc = val_acc
            save_cascade(save_path, model, extra={'best_val_acc': best_val_acc})
            print(f'  [OK] saved {save_path} (val acc {val_acc:.2f}%)')

    print(f'Cascade done. best val 5-class acc = {best_val_acc:.2f}%')
...
def parse_args():
    ...
    parser.add_argument('--data', type=str, default='分类数据')
