...
class ATCSANet(nn.Module):
    """CNN + CCSTA classifier used by every cascade node.

    The convolutional trunk is the original 4-layer CNN; CCSTA is inserted
    after the last convolution, as in the paper backbone diagram.
...
def _fc_head(out_dim, dropout=0.3):
...
class ATCSACascadeNet(nn.Module):
    """Shared CNN+CCSTA trunk with coarse / S1 / S2 heads (paper path-wise ACGC).

    Coarse head: 3 superclasses {0,4} | {2,3} | {1}
    S1 head: binary class 2 vs 3
    S2 head: binary class 0 vs 4
    """

    def __init__(self, use_ccsta=True, num_heads=8, chunk_size=2, dropout=0.3):
        super().__init__()
        self.use_ccsta = use_ccsta
        self.trunk = ATCSANet(
            num_classes=5, use_ccsta=use_ccsta,
            num_heads=num_heads, chunk_size=chunk_size, dropout=dropout,
        )
        self.head_coarse = _fc_head(3, dropout)
        self.head_s1 = _fc_head(1, dropout)
        self.head_s2 = _fc_head(1, dropout)

    def forward_heads(self, x):
        feat = self.trunk.extract_features(x)
        metric_feat = F.normalize(self.trunk.metric_head(feat), dim=1)
        return {
            'feat': feat,
            'metric': metric_feat,
            'coarse': self.head_coarse(feat),
            's1': self.head_s1(feat).view(-1),
            's2': self.head_s2(feat).view(-1),
        }

    def load_trunk_from(self, state_dict):
